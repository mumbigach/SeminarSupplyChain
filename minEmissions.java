 ...
 *
 */
public class minEmissions {

	public static void main(String[] args) throws IOException {
		//put the correct data
		FileInputStream file = new FileInputStream(new File("C:\\Users\\maken\\OneDrive - Erasmus University Rotterdam\\Documents\\UPT Master\\Seminar Supply Chain\\Data4Em.xlsx"));
		Workbook workbook = new XSSFWorkbook(file);
		// get the multiple sheets
		Sheet sheet1 = workbook.getSheet("PortEDCEm");
		Sheet sheet2 = workbook.getSheet("PortDCEm");
		Sheet sheet3 = workbook.getSheet("EDCDemandEm");
		Sheet sheet4 = workbook.getSheet("DCArbitraryEm");
		Sheet sheet5 = workbook.getSheet("Demand");
		Sheet sheet7 = workbook.getSheet("EDCcap");
		Sheet sheet8 = workbook.getSheet("WarehouseEmissions"); //in kilos
		...
		for (int j = 0; j < numLDCs; j++) {
				...
				warehouseEmissions[j] = sheet8.getRow(0).getCell(j).getNumericCellValue();
		...
		//	for(int i=0; i<capacity.length; i++) {
				solveModel(costPortEDC, costPortLDC, costEDCDemandRegion, costLDCArbitrary, demand, edcCap, warehouseEmissions);
		//	}
	...
	public static void solveModel(double[][] costPortEDC, double[][] costPortLDC, double[][] costEDCDemandRegion, double[] costLDCArbitrary,
			int[] demand, double[] edcCap, double[] warehouseEmissions) throws IloException {
		...
		// variable emissions Port -> EDC/LDC
		for(int i=0; i<numPorts; i++) {
			for(int j=0; j<numEDCs; j++) {
		...
		//variable emissions EDC to Demand region
		for(int i=0; i<numEDCs; i++) {
			for(int j=0; j<numDemandRegions; j++) {
				objExpr = cplex.sum(objExpr, cplex.prod(costEDCDemandRegion[i][j], z3[i][j]));
			}
		}
		//variable emissions LDC to arbitrary city
		for(int i=0; i<numLDCs; i++) {
			for(int j=0; j<numDemandRegions; j++) {
		...
		for(int j=0; j<numDemandRegions; j++) {
			for(int i=0; i<numLDCs; i++) {
				objExpr = cplex.sum(objExpr, cplex.prod(x2[i][j], warehouseEmissions[j]));
			}
			for(int i=0; i<numEDCs; i++) {
				objExpr = cplex.sum(objExpr, cplex.prod(x1[i][j], warehouseEmissions[j]));
			}		
		}
		...
		if (cplex.getStatus() == IloCplex.Status.Optimal) {			
			...
			System.out.println("Objective = " + cplex.getObjValue() + " kilos");
			System.out.println("Objective = " + cplex.getObjValue()/1000 + " tonnes");
			System.out.println("emissions cost = " + 99.8*cplex.getObjValue()/1000 + " euros");
