...
public class trucksOnly {
	...
	public static void main(String[] args) throws IOException {
		...
		FileInputStream file = new FileInputStream(new File("C:\\Users\\maken\\OneDrive - Erasmus University Rotterdam\\Documents\\UPT Master\\Seminar Supply Chain\\DataTruckOnly.xlsx"));
		...
		Sheet sheet8 = workbook.getSheet("WarehouseLDC");
	...
	public static void solveModel(double[][] costPortEDC, double[][] costPortLDC, double[] fixedEDC, double[][] variableEDC, double[] warehouseLDC, double[][] costEDCDemandRegion, double[] costLDCArbitrary,
		...
		//Constraint: For EDCs only
		for(int i=0; i<numLDCs; i++) {
		//	cplex.addEq(y2[i], 0);
		}
		//Constraint: For LDCs only
		for(int i=0; i<numEDCs; i++) {
		//	cplex.addEq(y1[i], 0);
		}
		...
		if (cplex.getStatus() == IloCplex.Status.Optimal) {
			...
			double val = cplex.getObjValue() + cplex.getValue(cplex.prod(-1*costPortLDC[0][19], z2[0][19]))+ cplex.getValue(cplex.prod(-1*costPortLDC[1][15], z2[1][15])) + cplex.getValue(cplex.prod(-1*costPortLDC[5][13], z2[5][13])) + cplex.getValue(cplex.prod(-1*costPortLDC[6][26], z2[6][26])) + cplex.getValue(cplex.prod(-1*costPortLDC[7][8], z2[7][8])) + cplex.getValue(cplex.prod(-1*costPortLDC[7][9], z2[7][9])) + cplex.getValue(cplex.prod(-1*costPortLDC[9][22], z2[9][22]));
